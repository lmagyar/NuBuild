//===========================================================================
// MODULE:  NuPrepareBuild.cs
// PURPOSE: NuBuild prepare MSBuild task for build
// 
// Copyright © 2012
...
namespace NuBuild.MSBuild
{
   ...
   /// </remarks>
   public sealed class NuPrepareBuild : Task
   {
         ...
         XDocument specDoc)
      {
         ...
         if (this.BuildNumber == 0)
         {
            ...
            var buildFile = Path.Combine(
               this.OutputPath,
               String.Format("{0}.build.number", Path.GetFileNameWithoutExtension(ProjectPath))
            );
